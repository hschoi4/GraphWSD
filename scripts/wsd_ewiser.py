...
class Ewiser(torch.nn.Module):
    def __init__(self, model_params, A=None, O=None, adjacency_trainable=False, renormalize=False):
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(model_params['lm'])#camembert-base)#flaubert/flaubert_large_cased)#camembert-base)
        self.bert = AutoModel.from_pretrained(model_params['lm'])#camembert-base#flaubert/flaubert_large_cased)#camembert-base)
        # encoder addition
        ...
        if A is not None:
            ...
            if not model_params['fragment']:
                ...
                self.vec = torch.nn.Parameter(torch.tensor([1.0 for _ in range(len(self.A))]), requires_grad=self.adjacency_trainable)
...
class Enet(Ewiser):
    def __init__(self, config, A=None):
        ...
        self.config = config
        self.labels = None
        self.baseline = Ewiser(self.config, A, renormalize=False, adjacency_trainable=config['adj_trainable']).to(self.config['device'])
    
    def train(self, traindata, testdata): 
        
        if self.config['load_model']: return
        ...
        early_stopping = utils.EarlyStopping(patience=5, verbose=True, save_path=f'runs/ewiser_{self.config["model_num"]}/checkpoint_{str(time.time())}.pt')
        ...
        for epoch in range(self.epochs):
            ...
            for idx, batch in enumerate(batches):
                ...
                if ((epoch*self.config['batch_size'])+idx) % 2 == 0:
                    print(f"Epoch: {epoch+1} Batch: {idx+1}/ {len(traindata.instances)// self.config['batch_size']}, Loss {loss.item()} \t Avg time: {t2-t1}seconds")
        ...
        if self.config['save_model']:
            torch.save(self.baseline.state_dict(), f"{self.config['save_model_dir']}/model_{self.config['model_num']}_e{epoch}_b{idx}.pth"); print('model saved !!')
#            if epoch == 1: break
    ...
    def predict(self, testdata, name=None):
        ...
        with torch.no_grad():
            ...
            for idx, batch in tqdm(enumerate(batches)):
                ...
                tks.extend(opt[0].detach().cpu().numpy()); tki.extend(opt[1].detach().cpu().numpy())
            ...
            fp = open(f"{self.config['save_model_dir']}ewiser_{self.config['model_num']}/predictions_{self.config['model_num']}_{name}.txt", 'w')
            ...
            fp.close()
            if 'test' in name:
                fp2 = open(f"{self.config['save_model_dir']}ewiser_{self.config['model_num']}/analyze_pred_{self.config['model_num']}_{name}.txt", 'w')
